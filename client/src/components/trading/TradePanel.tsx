...
export const TradePanel = memo(({ quotes, selectedSymbol, onSelectSymbol }: TradePanelProps) => {
  ...
  const bidLevels = (
    orderBookQuery.data?.bids ?? Array.from({ length: 3 }, () => ({ price: 0, size: 0 }))
  )
    .slice(0, 3)
    .map((level, idx) => ({
  ...
  const askLevels = (
    orderBookQuery.data?.asks ?? Array.from({ length: 3 }, () => ({ price: 0, size: 0 }))
  )
    .slice(0, 3)
    .map((level, idx) => ({
      ...
      <Stack spacing={2}>
        ...
        <Stack
          sx={(theme) => ({
            ...
            p: 1.75,
            borderColor: alpha(
              selectedQuote?.source === 'live' ? theme.palette.primary.main : theme.palette.divider,
              selectedQuote?.source === 'live' ? 0.28 : 0.9,
            ),
          })}
        >
          <Stack spacing={1.25}>
            ...
            <Stack direction="row" spacing={1.25}>
              <Typography
                variant="body2"
                sx={{
                  color: (selectedQuote?.changePercent ?? 0) >= 0 ? 'success.main' : 'error.main',
                  fontWeight: 700,
                }}
              >
                {selectedQuote
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25}>
          <ToggleButtonGroup
            ...
            color="primary"
          >
            <ToggleButton value="buy" disableRipple>
              Buy
            </ToggleButton>
            <ToggleButton value="sell" disableRipple>
              Sell
            </ToggleButton>
          </ToggleButtonGroup>
...
TradePanel.displayName = 'TradePanel';import {
  Alert,
  Box,
  Button,
  ButtonBase,
